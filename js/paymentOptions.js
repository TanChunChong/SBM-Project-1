  ...
  async function fetchCardData(userId, username) {
      try {
          ...
          if (cardDoc.exists()) {
              ...
              localStorage.setItem('cachedCardData', JSON.stringify(cardData));
  ...
  const cachedCardData = localStorage.getItem('cachedCardData');
  if (cachedCardData) {
      displayCardData(JSON.parse(cachedCardData));
  }
  ...
  document.addEventListener("DOMContentLoaded", function () {
      ...
      onAuthStateChanged(auth, async (user) => {
          if (user) {
              ...
              await fetchCardData(userId, username);
          } else {
